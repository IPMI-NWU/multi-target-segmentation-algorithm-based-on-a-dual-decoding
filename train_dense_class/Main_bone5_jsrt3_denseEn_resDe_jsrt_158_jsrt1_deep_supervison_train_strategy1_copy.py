...
def train(net, epoch, trainloader, optimizer, outTxtPath, type='bone', task=None):
    ...
    for iter in range(0, iters):
        for step, (imgs, mask_map1, mask_map2, mask_map3, mask, _) in enumerate(trainloader):
            ...
            optimizer.zero_grad()
            [output_d1, output_d2] = net(imgs) # 2为肋骨, output_d1 3 , output_d2 5

            loss = 0
            output_p = torch.zeros(output_d2[-1].shape[0], len(mask),
                                   output_d2[-1].shape[-2], output_d2[-1].shape[-1]).cuda()
            ...
            if type == 'bone':
                ...
                bone_weight = [1, 1, 1, 1]
                for loss_index in range(1, len(output_d2)):
                    if len(task_index) > 1:
                        for index_t in task_index: # 不同的官
                            loss += bone_weight[loss_index] * compute_multi_loss(output_d2[loss_index][:, index_t, :, :], mask_total[loss_index][index_t])
                            # loss = compute_multi_loss_dice_focal_bone(output_d2, mask)
                    else:
                        loss = bone_weight[loss_index] * compute_multi_loss(output_d2[loss_index][:, task_index[0], :, :],
            ...
            if type == 'others':
                ...
                loss = 0
                for loss_index in range(1, len(output_d1)):
                    if len(task_index) > 1:
                        for index_t in task_index: # 不同的官
                            loss += compute_multi_loss(output_d1[loss_index][:, index_t - 5, :, :], mask_total[loss_index][index_t - 5])
                            # loss = compute_multi_loss_dice_focal_bone(output_d2, mask)
                    else:
                        loss = compute_multi_loss(output_d1[loss_index][:, task_index[0] - 5, :, :],
                                                                             mask_total[loss_index][0])
                loss /= len(mask)
...
def val(net, epoch, testloader, outTxtPath, type='bone', task=None):
    if task is None:
        task = ['pre_rib', 'post_rib', 'clavicel', 'all_bone']
    net.eval()
    test_loss = 0

    # dice, iou(jaccard), precision, recall = 0, 0, 0, 0
    task_total_list = ['pre_rib', 'post_rib', 'clavicel', 'all_bone', 'vindr_rib', 'lung', 'heart', 'jsrtClavicel']
    dice = [0, 0, 0, 0, 0, 0, 0, 0]
    iou = [0, 0, 0, 0, 0, 0, 0, 0]
...
def one_step(i, outTxtPath_bone,  outTxtPath_jsrt, num_fold_path_bone, num_fold_path_jsrt, file_name, model):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 4 == 0 and test_dice_ >= best_dice_bone and test_dice_ > 0.82:
        ...
        if epoch % 4 == 2 and test_dice_ >= best_dice_vindr_rib and test_dice_ > 0.82:
            # torch.save(net.state_dict(), dir)
        ...
        if epoch % 4 == 3 and test_dice_ >= best_dice_jsrt and test_dice_ > 0.82:
            # torch.save(net.state_dict(), dir)
        ...
        if epoch % 4 == 1 and test_dice_ >= best_dice_mg and test_dice_ > 0.82:
            # torch.save(net.state_dict(), dir)
...
def main1():
    from dense_class.encoder_densenet_121_1center_bone_2CAS_jsrt_2AG_bone_AMFS_jsrt_AMFS137_deep_supervision import densenet121_resDecoder

    model = densenet121_resDecoder(pretrained=True, num_classes_d1=3, num_classes_d2=4)
    file_name = '2023_2_5_bone4_jsrt3_denseEn_resDe_two_decoder_1center_denseblock_4fold_bone_2CAS_jsrt_2AG_bone_AMFS_jsrt_AMFS137_deep_supervision_map2_map3_bone_0.6_0.4_1'

    return file_name, model

def main2():
    from dense_class.encoder_densenet_121_1center_bone_2CAS_jsrt_2AG_bone_AMFS_jsrt_AMFS137_deep_supervision_map2_map3_weight_std import densenet121_resDecoder

    model = densenet121_resDecoder(pretrained=True, num_classes_d1=3, num_classes_d2=5)
    file_name = '2023_2_14_mg10_jsrt5_vindr5_bone10_aug_epoch150_bone5_others3_denseEn_resDe_two_decoder_1center_denseblock_4fold_bone_2CAS_jsrt_2AG_bone_AMFS_jsrt_AMFS137_deep_supervision_map2_map3_weight_std'

    return file_name, model


def main3():
    from dense_class.encoder_densenet_121_1center_bone_2CAS_jsrt_2AG_bone_AMFS_jsrt_AMFS137_deep_supervision_map2_map3_weight_std_jsrt_no_std import densenet121_resDecoder

    model = densenet121_resDecoder(pretrained=True, num_classes_d1=3, num_classes_d2=5)
    file_name = '2023_2_10_bone4_jsrt3_denseEn_resDe_two_decoder_1center_denseblock_4fold_bone_2CAS_jsrt_2AG_bone_AMFS_jsrt_AMFS137_deep_supervision_map2_map3_weight_std_jsrt_no_std'
