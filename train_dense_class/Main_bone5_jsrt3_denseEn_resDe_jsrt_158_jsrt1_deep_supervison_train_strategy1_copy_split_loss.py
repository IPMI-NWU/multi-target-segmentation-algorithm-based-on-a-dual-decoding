...
# 计算损失
def compute_multi_loss_dce(output, mask):
    from utils.LossAndEval import DiceLoss
    criterion2 = DiceLoss()

    loss_mask = criterion2(output.squeeze(), mask.squeeze())

    return loss_mask

def compute_multi_loss_new(output, mask):
    from utils.LossAndEval import DiceLoss, FocalLoss
    criterion2 = DiceLoss()
    criterion3 = FocalLoss()  # TopKLoss is not callable 变量名重用了

    loss_mask = criterion2(output.squeeze(), mask.squeeze()) + criterion3(output.squeeze(), mask.squeeze())
        ...
        for step, (imgs, mask_map1, mask_map2, mask_map3, mask, _) in enumerate(trainloader):
                ...
                for loss_index in range(1, len(output)):
                    if len(task_index) > 1:
                        for index_t in task_index: # 不同的官
                            loss += bone_weight[loss_index] * compute_multi_loss_new(output[loss_index][:, index_t, :, :], mask_total[loss_index][index_t])
                            # loss = compute_multi_loss_dice_focal_bone(output_d2, mask)
                    else:
                        loss = bone_weight[loss_index] * compute_multi_loss_new(output[loss_index][:, task_index[0], :, :], mask_total[loss_index][0])
                loss /= len(mask)
            ...
            if type == 'others':
                ...
                for loss_index in range(1, len(output)):
                    if len(task_index) > 1:
                        for index_t in task_index: # 不同的官
                            loss += compute_multi_loss_new(output[loss_index][:, index_t - 5, :, :], mask_total[loss_index][index_t - 5])
                            # loss = compute_multi_loss_dice_focal_bone(output_d2, mask)
                    else:
                        loss = compute_multi_loss_new(output[loss_index][:, task_index[0] - 5, :, :], mask_total[loss_index][0])
                loss /= len(mask)
...
def val(net, epoch, testloader, outTxtPath, type='bone', task=None):
    ...
    with torch.no_grad():
        for step, (imgs, mask_map1, mask_map2, mask_map3, mask, _) in enumerate(testloader):
            ...
            loss = 0
            for i in range(len(mask)):
                loss = compute_multi_loss_new(output_p[:, i, :, :], mask[i])
...
def main1():
    ...
    file_name = '2023_2_16_topkloss_bone5_jsrt3_split_denseEn_resDe_two_decoder_1center_denseblock_4fold_bone_2CAS_jsrt_2AG_bone_AMFS_jsrt_AMFS137_deep_supervision_map2_map3_bone_0.6_0.4_1'
