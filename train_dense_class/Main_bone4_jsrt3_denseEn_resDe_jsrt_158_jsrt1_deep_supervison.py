...
def train(net, epoch, trainloader, optimizer, outTxtPath, type='bone'):
    ...
    for iter in range(0, iters):
        for step, (imgs, mask_map1, mask_map2, mask_map3, mask, _) in enumerate(trainloader):
            ...
            optimizer.zero_grad()
            [output_d1, output_d2] = net(imgs) # 2为肋骨
            ...
            if type == 'bone':
                ...
                for loss_index in range(1, len(output_d2)):
                    loss += bone_weight[loss_index] * compute_multi_loss(output_d2[loss_index], mask_total[loss_index])
            ...
            if type == 'jsrt':
                ...
                for loss_index in range(1, len(output_d1)):
                    loss += compute_multi_loss(output_d1[loss_index], mask_total[loss_index])
            ...
            if type == 'bone' and step % 8 == 0:
                shuchu = 'Epoch: {:d} iter: {:d} step: {:d}  Train loss: {:.6f} dice_labelPrerib: {:.6f} dice_labelPosteriorrib: {:.6f}  dice_labelClavicel: {:.6f} dice_all_bone: {:.6f} lr:{:.12f}'.format(
...
def main2():
    ...
    model = densenet121_resDecoder(pretrained=True, num_classes_d1=3, num_classes_d2=4)
    # file_name = '2023_2_6_7_bone4_jsrt3_denseEn_resDe_two_decoder_1center_denseblock_4fold_bone_2CAS_jsrt_2AG_bone_AMFS_jsrt_AMFS137_deep_supervision_map2_map3_weight_std'

    file_name = '2023_2_12_bone4_jsrt3_denseEn_resDe_two_decoder_1center_denseblock_4fold_bone_2CAS_jsrt_2AG_bone_AMFS_jsrt_AMFS137_deep_supervision_map2_map3_weight_std'
    return file_name, model
...
def main3():
    ...
    model = densenet121_resDecoder(pretrained=True, num_classes_d1=3, num_classes_d2=4)
    file_name = '2023_2_7_bone4_jsrt3_denseEn_resDe_two_decoder_1center_denseblock_4fold_bone_2CAS_jsrt_2AG_bone_AMFS_jsrt_AMFS137_deep_supervision_map2_map3_weight_std_jsrt_no_std'

    return file_name, model

if __name__ == "__main__":
    # 文档的输出位置
    outModelPath_dir = r'../../compare_experiment/OUT/no_contour/bone_jsrt_dense_class'
