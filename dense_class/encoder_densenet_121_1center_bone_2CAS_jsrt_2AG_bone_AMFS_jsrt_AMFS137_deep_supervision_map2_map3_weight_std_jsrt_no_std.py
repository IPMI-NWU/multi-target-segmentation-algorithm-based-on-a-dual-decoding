...
class Up(nn.Module):
    def __init__(self, in_size, out_size, is_deconv=False, weight_std=False):
        ...
        if is_deconv:
            ...
            self.up = nn.Sequential(
                ...
                conv2d_3x3(in_size, out_size, 1, padding=0, weight_std=weight_std)
            )
...
class DenseNet_resDecoder(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16), num_init_features=64,
                 bn_size=4, compression_rate=0.5, drop_rate=0.2, n_classes_d1=2, n_classes_d2=4):
        """
        ...
        self.conv1 = nn.Sequential(OrderedDict([
            ("conv0", conv2d_3x3(3, num_init_features, kernel_size=3, stride=1, padding=1, bias=False, weight_std=self.weight_std)),
        ...
        is_deconv = False
        # 解码器1
        self.ASPPBlock_d1 = ASPPBlock_137(2 * channels[3], weight_std=False)
        self.fusionConv_d1 = nn.Sequential(
            nn.BatchNorm2d(2 * channels[3]),
            nn.ReLU(inplace=True),
            conv2d_3x3(2 * channels[3], 2 * channels[3], kernel_size=3, padding=1, weight_std=False),
        )

        self.up1_d1 = Up(2 * channels[3], channels[3], is_deconv, weight_std=False)
        self.d1_resb_d1 = self._make_layer(NoBottleneck, 2 * channels[3], channels[3], 1, stride=1, weight_std=False)

        self.up2_d1 = Up(channels[3], channels[2], is_deconv, weight_std=False)
        self.d2_resb_d1 = self._make_layer(NoBottleneck, 2 * channels[2], channels[2], 1, stride=1, weight_std=False)

        self.up3_d1 = Up(channels[2], channels[1], is_deconv, weight_std=False)
        self.d3_resb_d1 = self._make_layer(NoBottleneck, 2 * channels[1], channels[1], 1, stride=1, weight_std=False)

        self.up4_d1 = Up(channels[1], channels[0], is_deconv, weight_std=False)
        self.d4_resb_d1 = self._make_layer(NoBottleneck, 2 * channels[0], channels[0], 1, stride=1, weight_std=False)

        # 解码器1-跳跃连接
        self.skip0_att_d1 = Attention_block(channels[0], channels[0], channels[0] // 2, weight_std=False)
        self.skip1_att_d1 = Attention_block(channels[1], channels[1], channels[1] // 2, weight_std=False)
        ...
        self.map1_d1 = nn.Sequential(
            # nn.Upsample(scale_factor=8, mode='bilinear'),
            conv2d_3x3(channels[3], channels[0], kernel_size=3, padding=1, weight_std=False),
            nn.BatchNorm2d(channels[0]),
            nn.ReLU(inplace=True),
            nn.Conv2d(channels[0], n_classes_d1, kernel_size=1)
        )

        self.map2_d1 = nn.Sequential(
            # nn.Upsample(scale_factor=4, mode='bilinear'),
            conv2d_3x3(channels[2], channels[0], kernel_size=3, padding=1, weight_std=False),
            nn.BatchNorm2d(channels[0]),
            nn.ReLU(inplace=True),
            nn.Conv2d(channels[0], n_classes_d1, kernel_size=1)
        )

        self.map3_d1 = nn.Sequential(
            # nn.Upsample(scale_factor=2, mode='bilinear'),
            conv2d_3x3(channels[1], channels[0], kernel_size=3, padding=1, weight_std=False),
            nn.BatchNorm2d(channels[0]),
            nn.ReLU(inplace=True),
            nn.Conv2d(channels[0], n_classes_d1, kernel_size=1)
        )
        ...
        # 解码器2 bone
        self.ASPPBlock_d2 = ASPPBlock(2 * channels[3], weight_std=self.weight_std)
        self.fusionConv_d2 = nn.Sequential(
            nn.BatchNorm2d(2 * channels[3]),
            nn.ReLU(inplace=True),
            conv2d_3x3(2 * channels[3], 2 * channels[3], kernel_size=3, padding=1, weight_std=self.weight_std),
        )

        self.up1_d2 = Up(2 * channels[3], channels[3], is_deconv, weight_std=self.weight_std)
        self.d1_resb_d2 = self._make_layer(NoBottleneck, 2 * channels[3], channels[3], 1, stride=1, weight_std=self.weight_std)

        self.up2_d2 = Up(channels[3], channels[2], is_deconv, weight_std=self.weight_std)
        self.d2_resb_d2 = self._make_layer(NoBottleneck, 2 * channels[2], channels[2], 1, stride=1, weight_std=self.weight_std)

        self.up3_d2 = Up(channels[2], channels[1], is_deconv, weight_std=self.weight_std)
        self.d3_resb_d2 = self._make_layer(NoBottleneck, 2 * channels[1], channels[1], 1, stride=1, weight_std=self.weight_std)

        self.up4_d2 = Up(channels[1], channels[0], is_deconv, weight_std=self.weight_std)
        self.d4_resb_d2 = self._make_layer(NoBottleneck, 2 * channels[0], channels[0], 1, stride=1, weight_std=self.weight_std)

        # 解码器2-跳跃连接
        self.skip0_att_d2 = CAS(channels[0], channels[0], channels[0] // 2, weight_std=self.weight_std)
        self.skip1_att_d2 = CAS(channels[1], channels[1], channels[1] // 2, weight_std=self.weight_std)
        # self.skip2_att_d2 = CAS(channels[2], channels[2], channels[2] // 2)

        self.cls_conv_d2 = nn.Conv2d(channels[0], n_classes_d2, kernel_size=1)

        self.map1_d2 = nn.Sequential(
            # nn.Upsample(scale_factor=8, mode='bilinear'),
            conv2d_3x3(channels[3], channels[0], kernel_size=3, padding=1, weight_std=self.weight_std),
            nn.BatchNorm2d(channels[0]),
            nn.ReLU(inplace=True),
            nn.Conv2d(channels[0], n_classes_d2, kernel_size=1)
        )

        self.map2_d2 = nn.Sequential(
            # nn.Upsample(scale_factor=4, mode='bilinear'),
    ...
    def forward(self, x):
        ...
        output_map1_d1 = self.map1_d1(d1_d1)
        output_map2_d1 = self.map2_d1(d2_d1)
        output_map3_d1 = self.map3_d1(d3_d1)
        output_d1 = self.cls_conv_d1(d4_d1)

        # 解码器2
        center_d2 = self.ASPPBlock_d2(x)
        center_d2 = self.fusionConv_d2(center_d2)
        x_d2 = self.up1_d2(center_d2)
        ...
        output_map1_d2 = self.map1_d2(d1_d2)
        output_map2_d2 = self.map2_d2(d2_d2)
        output_map3_d2 = self.map3_d2(d3_d2)

        output_d2 = self.cls_conv_d2(d4_d2)

        output_deep_d1 = [output_map1_d1, output_map2_d1, output_map3_d1, output_d1]
        output_deep_d2 = [output_map1_d2, output_map2_d2, output_map3_d2, output_d2]
        return [output_deep_d1, output_deep_d2]


    def _make_layer(self, block, inplanes, planes, blocks, stride=1, dilation=1, multi_grid=1, att=False, weight_std=False):
        ...
        if stride != 1 or inplanes != planes:  # 步长不为1， 输入与输出不等
            downsample = nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                conv2d_3x3(inplanes, planes, kernel_size=1, stride=stride, padding=0, weight_std=weight_std),
            )
        ...
        if stride == 1 and att:
            layers.append(block(inplanes, planes, stride, dilation=dilation, downsample=downsample,
                                multi_grid=generate_multi_grid(0, multi_grid), weight_std=weight_std, att=att))
        else:
            layers.append(block(inplanes, planes, stride, dilation=dilation, downsample=downsample,
                                multi_grid=generate_multi_grid(0, multi_grid), weight_std=weight_std))
...
def densenet121_resDecoder(pretrained=True, num_classes_d1=3, num_classes_d2=4, **kwargs):
    """DenseNet121"""
    print('using densenet121_resDecoder 1center-denseblock bone 2CAS jsrt 2AG 2layer bone AMFS jsrt AMFS137 deep supervision')
