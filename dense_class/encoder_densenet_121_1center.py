...
class DenseNet_resDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        skip1_d1 = torch.cat((skip1_low, x_d1), dim=1)
        d3_d1 = self.d3_resb_d1(skip1_d1)

        x_d1 = self.up4_d1(d3_d1)
        skip0_d1 = torch.cat((skip0_low, x_d1), dim=1)
        ...
        skip1_d2 = torch.cat((skip1_low, x_d2), dim=1)
        d3_d2 = self.d3_resb_d2(skip1_d2)

        x_d2 = self.up4_d2(d3_d2)
        skip0_d2 = torch.cat((skip0_low, x_d2), dim=1)
...
def densenet121_resDecoder(pretrained=True, num_classes_d1=3, num_classes_d2=4, **kwargs):
    """DenseNet121"""
    print('using densenet121_resDecoder 1center-denseblock')
