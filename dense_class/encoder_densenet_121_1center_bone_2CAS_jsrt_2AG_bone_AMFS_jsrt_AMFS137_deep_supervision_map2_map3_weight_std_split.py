...
class Decoder_d1(nn.Module):
    def __init__(self, inplanes, weight_std, channels, is_deconv, n_classes_d1):
        ...
        # 解码器1
        self.ASPPBlock_d1 = ASPPBlock_137(2 * channels[3])
        ...
        # 解码器1-跳跃连接
        self.skip0_att_d1 = Attention_block(channels[0], channels[0], channels[0] // 2, weight_std=self.weight_std)
        self.skip1_att_d1 = Attention_block(channels[1], channels[1], channels[1] // 2, weight_std=self.weight_std)
...
class Decoder_d2(nn.Module):
    def __init__(self, inplanes, weight_std, channels, is_deconv, n_classes_d2):
        ...
        # 解码器2 bone
        self.ASPPBlock_d2 = ASPPBlock(2 * channels[3])
...
class DenseNet_resDecoder(nn.Module):
    ...
    def forward(self, x, mode='train', task_type='bone'):
        ...
        if mode == 'train':
            if task_type == 'bone':
                output_deep_d2 = self.deconder_d2(x, skip3_low, skip2_low, skip1_low, skip0_low)
                return output_deep_d2

            if task_type == 'others' or task_type == 'jsrt':
...
def densenet121_resDecoder(pretrained=True, num_classes_d1=3, num_classes_d2=4, **kwargs):
    """DenseNet121"""
    print('using densenet121_resDecoder 1center-denseblock bone 2CAS jsrt 2AG 2layer bone AMFS jsrt AMFS137 deep supervision weight split')
