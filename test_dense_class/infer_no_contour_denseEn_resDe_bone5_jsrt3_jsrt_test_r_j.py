...
def save_img(output, img_name, fold, dir_name, task_type):
    path = r'../../result_bone5_JSRT3_jsrt_r_j/' + dir_name + ''

    mkdir(path)

    path = os.path.join(path, 'jsrt_fold_' + str(fold), task_type)
    mkdir(path)
...
def infer(testloader, outTxtPath, fold, model_index, dir_name, type='bone'):
    ...
    model_pth = [
        ...
        [
            [
                'r_j/jsrt/model_jsrt_81-0.955277.pth',
            ],
        ],
    ]
