...
def save_img(output, img_name, fold, dir_name, task_type):
    path = r'../../result_bone5_JSRT3_bone_r_j/' + dir_name + ''
    ...
    seg_result_name = ['all_bone', 'clavicel', 'post_rib', 'pre_rib', 'lung', 'heart', 'jsrtClavicel']
...
def infer(testloader, outTxtPath, fold, model_index, dir_name, type='bone'):
    ...
    model_pth = [
        ...
        [
            [
                'r_j/bone/fold_0/model_bone_44-0.906042.pth',
            ],

            [
                'r_j/bone/fold_1/model_bone_48-0.897802.pth',
            ],

            [
                'r_j/bone/fold_2/model_epoch_50-0.901866.pth',
            ],

            [
                'r_j/bone/fold_3/model_bone_58-0.898649.pth',
            ],
        ],
    ]
    ...
    labels_name = ['all_bone', 'clavicel', 'post_rib', 'pre_rib',  'lung', 'heart', 'jsrtClavicel']
    with torch.no_grad():
        for step, (imgs, mask_map1, mask_map2, mask_map3, mask, file_name) in enumerate(testloader):
            ...
            for i in range(len(labels_name)):
                if i < 4:
                    ...
                    output_dict[labels_name[i]] = output_d1[-1][:, i - 4, :, :] # jsrt
            ...
            for i in range(len(labels_name)):
                if i < 4:
                    ...
                    masks_probs.append(F.sigmoid(output_d1[-1][:, i - 4, :, :]))
