...
# 保存图像, path为保存图片的父父目录，reg_result_name为8个类，img_name为test中的文件名
def save_img(output, img_name, fold, dir_name, task_type):
    path = r'../../result_bone5_JSRT3_mg_r_j_v_m/' + dir_name + ''

    mkdir(path)

    path = os.path.join(path, 'mg_fold_' + str(fold), task_type)
    mkdir(path)
...
def infer(testloader, outTxtPath, fold, model_index, dir_name, type='bone'):
    ...
    model_pth = [
        ...
        [
            # FOLD_0
            [
                'r_j_v_m/mg/fold_0/model_mg_87-0.977068.pth',
            ],
            # fold_1
            [
                'r_j_v_m/mg/fold_1/model_mg_19-0.982375.pth',
            ],
            # mg fold_2
            [
                'r_j_v_m/mg/fold_2/model_mg_135-0.980178.pth',
            ],
        ],
    ]
...
if __name__ == '__main__':
    ...
    shuchu_list = \
        [
            # 0
            'bone5_others3_split_mg_r_j_v_m',
        ]
    ...
    for i in range(0, num):
        ...
        if type_task == 'jsrt':
            test_img_label_txt = r'../../dataset/Montgomery/lung_512_square/' + str(i+1) + '_fold/txt_mg' + '/test.txt' #r'../../dataset/' + type_task + '/txt_3organ_train158/' + 'test.txt'
        if type_task == 'bone':
