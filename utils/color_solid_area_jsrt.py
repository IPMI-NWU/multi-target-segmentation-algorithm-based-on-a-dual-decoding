...
def save_solid_pre_gt(file_name, fold_name, order_list=['all_bone', 'bone_contour']):
    ...
    gt_root = r'../../dataset/jsrt/image_mask/test'
...
if __name__ == '__main__':
    pre_order_list = ['heart', 'lung', 'jsrtClavicel']  # 预测图像
    gt_order_list = ['heart', 'lung', 'clavicle']  #

    fold_name = ['jsrt_fold_0']  #

    result_fold = ['result_bone5_JSRT3_jsrt_r_j/bone5_others3_split_jsrt_r_j', 'result_bone5_JSRT3_jsrt_r_v_j_m/bone5_others3_split_jsrt_r_j_v_m']

    for fold_dir in result_fold:
        for fold in fold_name:
            path = os.path.join('../../', fold_dir,
                                fold, 'jsrt', pre_order_list[0])
